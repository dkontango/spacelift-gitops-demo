// Capture the GitHub PR + Spacelift run steps (14, 15, 16) live with the cursor.
async (page) => {
    ...
    window.__wiz = {
      ...
      find(sel,text){ let el=null;
        ...
        if(!el && text){ const t=text.toLowerCase();
          const all=document.querySelectorAll('button,a,span,div,td,th,h1,h2,h3,label,code,li,input,strong');
  ...
  async function captureStep(step, url, targets, redact) {
    await page.goto(url, { waitUntil: 'domcontentloaded' });
    await sleep(4000);
    await page.evaluate(HARNESS);
    const red = redact ? await page.evaluate(() => window.__wiz.redact()) : 0;
    ...
    await shot(); await shot();
    return { step, frames: fi, redacted: red, found: pts.map(p=>p.label) };
  }
  const GH = 'https://github.com/dkontango/spacelift-gitops-demo';
  const results = [];
  // Step 14 — the PR checks (DENY): PR #3 conversation, scrolled to the merge box / checks
  results.push(await captureStep('14', `${GH}/pull/3`, [
    {label:'pr-title', text:'flip sandbox bucket public'},
    {label:'sandbox-check', text:'spacelift-gitops-demo-sandbox'},
  ], true));
  // Step 15 — the deny detail on the Spacelift run (denied by plan policy)
  results.push(await captureStep('15',
    'https://dkontango.app.us.spacelift.io/stack/spacelift-gitops-demo-sandbox/run/01KXY8HZZWD5TS3Y9PQBPTB2Z3', [
    {label:'denied', text:'denied by plan policy'},
    {label:'deny-msg', text:'S3 public access is not allowed'},
  ], true));
  // Step 16 — PR checks pass after the fix (PR #4, which passed + merged)
  results.push(await captureStep('16', `${GH}/pull/4`, [
    {label:'pr4-title', text:'bump sandbox keeper'},
    {label:'merged', text:'Merged'},
  ], true));
  return results;
}
